from lab8 import PCX_parser, cat256, _200001, canvas_printer
...
def rgr(_in, out, canvas_maker):
    ...
    gbr_palette = tuple(bytes(color)[::-1] for color in palette) + (b"\0\0\0",) * (16 - clr_used) # здесь же флип с rgb в gbr
    assert len(gbr_palette) == 16
    ...
    for color in gbr_palette: write(color + b"\0")

    put_pixel = canvas_maker(len(matrix[0]), H)
    for y in range(H -1, -1, -1): # перевёрнутый Y
        ...
        for x in range(0, W, 2):
            ...
            color2 = VGA_filtered[row[xx]]
            buffer[pos] = color << 4 | color2
            pos += 1

            put_pixel(x, y, palette[color])
            put_pixel(xx, y, palette[color2])
        write(buffer)
...
def solve():
    with open(cat256, "rb") as _in:
        with open("rgr_cat16.bmp", "wb") as out:
            @canvas_printer
            def decor(canvas_maker):
                rgr(_in, out, canvas_maker)
            decor()
